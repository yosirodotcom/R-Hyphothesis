...
alpha <- 0.05
data <- c(9, 8.5, 7.6, 9.8, 10.5, 11.8, 10.9, 9.5, 7.6, 7.8, 8.7, 9.2, 10.6, 10, 11, 11.5, 7.5, 8.6, 7.8, 8.5)
mu <- 10
...
if (hyp_mode == "two-sided") {
  ...
  if (t_value < critical_value) {
    conclusion_text <- paste0("Because t_stat = ", round(t_value, 3), "\ninside critical region,\nreject H_0,\nthe mean sample is less\nthan to mean population")
  } else {
    conclusion_text <- paste0("Because t_stat = ", round(t_value, 3), "\noutside critical region,\nfail to reject H_0,\nthe mean sample is less\nthan to mean population")
  }
} else if (hyp_mode == "right") {
  ...
  if (t_value > critical_value) {
    conclusion_text <- paste0("Because t_stat = ", round(t_value, 3), "\ninside critical region,\nreject H_0,\nthe mean sample is greater\nthan to mean population")
  } else {
    conclusion_text <- paste0("Because t_stat = ", round(t_value, 3), "\noutside critical region,\nfail to reject H_0,\nthe mean sample is not greater\nthan to mean population")
  }
}
